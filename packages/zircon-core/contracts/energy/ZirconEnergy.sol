...
//  using SafeMath for uint256;
  ...
  function initialize(address _pylon, address _pair, address _token0, address _token1) external override {
  ...
  function getFeeByGamma(uint gammaMulDecimals) _initialize external override view returns (uint amount) {
    (uint _minFee, uint _maxFee) = getFee();
    uint _gammaHalf = 5e17;
    uint x = (gammaMulDecimals > _gammaHalf) ? (gammaMulDecimals - _gammaHalf) * (10) : (_gammaHalf - gammaMulDecimals) *(10);
    if (gammaMulDecimals <= 45e16 || gammaMulDecimals >= 55e16) {
      amount = _minFee + (_maxFee * (x) * (x))/(25e36); //25 is a reduction factor based on the 0.45-0.55 range we're using.
    } else {
      amount = (_minFee * (x) * (x) * (36)/(1e36)) + (_minFee); //Ensures minFee is the lowest value.
    }
  }


  function migrateLiquidity(address newEnergy) external override{
