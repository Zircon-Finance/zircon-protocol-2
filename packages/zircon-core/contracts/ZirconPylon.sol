...
contract ZirconPylon is IZirconPylon, ReentrancyGuard {
    ...
    function _handleSyncAndAsync(uint _amountIn, uint _pairReserveTranslated, uint _reserve, uint _balance, bool _isAnchor) private returns (uint liquidity, uint amountOut) {
        ...
        if (max > _reserve) {
            ...
            if (_amountIn <= freeSpace) {
                ...
                //Calculates Pylon pool tokens to mint
        ...
        //Second part of the function if have both sync + async minting to do
        console.log("Async Minting-> amountIn: freeSpace::", _amountIn, freeSpace);
        //What doesn't go into sync pool is thrown into liquidity pool with mintOneSide
        ...
        console.log("Async Minting Pylon liquidity:", liquidity);
    ...
    function burnAsync(address _to, bool _isAnchor) external nonReentrant isInitialized returns (uint amount0, uint amount1) {
...

        {
            ...
            ptu = payBurnFees(ptu);
            //console.log("ZPBa, ptu after fees:", ptu);
