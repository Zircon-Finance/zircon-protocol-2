...
const {casesSDK} = require("./shared/generate-json-sdk-test");
const {loadFixture} = require("@nomicfoundation/hardhat-network-helpers");
const {initData} = require("./shared/commands");
...
describe("Pair", () => {
    ...
    it('mintOneSide', async () => {
        ...
        let expectedLiquidity = ethers.BigNumber.from('14136477637657833468')
    ...
    it('mintOneSide 2', async () => {
        ...
        let expectedLiquidity = ethers.BigNumber.from('10049796017830411666')
    ...
    it.only('Burn One Side', async () => {
        ...
        let pairBalance = await pair.balanceOf(account.address)
        let token0Balance = await token0.balanceOf(account.address)
        let token1Balance = await token1.balanceOf(account.address)
        console.log("balances", token0Balance.toString(), token1Balance.toString(), pairBalance.toString())
        //await pair.mint(account.address)
