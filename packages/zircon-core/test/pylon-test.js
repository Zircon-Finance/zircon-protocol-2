...
function log(text, moreText = "", evenMoreText = "")  { console.log("\x1b[36m%s\x1b[0m", text, moreText, evenMoreText); }
...
describe("Pylon", () => {
    ...
    const printValues = async () => {
        ...
        log("lastK: ", lastK.toString())

        let price0CumulativeLast = await pair.price0CumulativeLast()
        log("price0CumulativeLast: ", price0CumulativeLast.toString())

        let price1CumulativeLast = await pair.price1CumulativeLast()
        log("price1CumulativeLast: ", price1CumulativeLast.toString())
        ...
        // let eInstance = await energyContract.attach(energyAddress)

        let ptbEnergy = await pair.balanceOf(energyAddress);
        log("PTB Energy", ptbEnergy.toString())

        let aEnergy = await token1.balanceOf(energyAddress)
        log("Anchor Energy", aEnergy.toString())

        console.log("\x1b[34m%s\x1b[0m", "END VALUES")
    }
    ...
    //         let derVfbSync = derVfb.add(pylonRes[0]);
    //
    //
    //         console.log("derVfb before test mint", ethers.utils.formatEther(derVfb));
    //         console.log("derVfb sync before test mint", ethers.utils.formatEther(derVfbSync));
    //         console.log("----tpv before test mint-----", ethers.utils.formatEther(tpv));
    //
    //
    //         if (isAnchor) {
    ...
    //
    //         //second fake mint to update
    //         await ethers.provider.send("hardhat_mine", ['0x30']);
    //
    //         await token0.transfer(pylonInstance.address, ethers.BigNumber.from("10"))
    ...
    //         derVfbSync = derVfb.add(pylonRes[0]);
    //
    //         console.log("derVfb after test mint", ethers.utils.formatEther(derVfb));
    //         console.log("derVfb sync after test mint", ethers.utils.formatEther(derVfbSync));
    //         console.log("----tpv after test mint----", ethers.utils.formatEther(tpv));
    //
    //
    ...
    //         // Anchor
    //
    //         // Forcing a bit the mint async
    //         await feeToSetter.setFees(0, ethers.BigNumber.from("40000000000000000"), 100, 240, 3, 2);
    //         if (isAnchor) {
    //             let t = token0Amount.div(100)
    //             console.log("Mint test token1Amount for second mint: ", ethers.utils.formatEther(t));
    //             console.log("Mint test token1Amount for second mint: ", t.toString());
    ...
    //
    //     const mintCase = [
    //         [expandTo18Decimals(10), expandTo18Decimals(5), '474999999999999999', '337490000000000000','99999999999999000', '149366473384710075', true],]
    //
    ...
    //     expect(vab).to.eq(ethers.BigNumber.from("53028130037333209843"));
    //
    //
    // });
    //
    ...
    //     expect(mu).to.eq(ethers.BigNumber.from("450347813923342395"));
    //
    //
    // });
    //
    ...
    //
    //     expect(thisBlockEMA).to.eq(ethers.BigNumber.from("99026859131494705"));
...
//         await token0.transfer(pylonInstance.address, token1Amount.mul(pairRes[0]).div(pairRes[1]))
//         await token1.transfer(pylonInstance.address, token1Amount)
//         console.log("token0Amount: ", token1Amount.mul(pairRes[0]).div(pairRes[1]).toString());
...
//
    it('Omega test', async function () {
        ...
        console.log("SENDING ====> ", aptBalance.div(100).toString())
        ...
        expect(balancePostBurn.sub(balancePreBurn)).to.eq(ethers.BigNumber.from('1055156096327655388'))
        ...
        expect(balancePostBurn.sub(balancePreBurn)).to.eq(ethers.BigNumber.from('903408502616276971'))
...
//
//
//     it('AnchorK Sync Test', async function () {
//         let token0Amount = expandTo18Decimals(1700)
//         let token1Amount = expandTo18Decimals(5300)
//         await addLiquidity(token0Amount, token1Amount)
//
...
//         for(let i = 0; i < 30; i++) {
//             pairResT = await pair.getReserves();
//             let floatAdd = (pairResT[0].mul(ptb).div(ptt)).div(50)
//             let anchorAdd = (pairResT[1].mul(ptb).div(ptt)).div(50)
//             ptb = await pair.balanceOf(pylonInstance.address);
//             ptt = await pair.totalSupply();
//
...
//         console.log("Floats received: ", ethers.utils.formatEther(floatsReceived));
//
//         expect(floatsReceived).to.eq(ethers.BigNumber.from('1683324077283482922960'));
//
//     });
//
//
...
//
    it('AnchorK float add/remove async test', async function () {
        ...
        console.log("value::", initialFloatPtBalance.div(3).toString())
        await printValues()
    ...
    //
    //     console.log("token0 sending", pairResk[0].div(10).toString())
    //     console.log("token1 sending", pairResk[1].div(10).toString())
    //
    ...
    //
    //     console.log("sent update floats:", ethers.utils.formatEther(token0Amount.div(10000)));
    //     console.log("sent update floats:", token0Amount.div(10000).toString());
    //     await token0.transfer(pylonInstance.address, token0Amount.div(10000))
    //     await pylonInstance.mintPoolTokens(account.address, false);
    //
    //
    ...
    //     expect(balancePostBurn.sub(balancePreBurn)).to.eq(ethers.BigNumber.from('5408439995651623595'))
    //
    //
    //
    //     //Now we add some float liquidity and see if the amount withdrawn is the same
    ...
    //     console.log("sent float PTs:", token0Amount.mul(1).toString());
    ...
    //     console.log("sent anchor value:", token1Amount.mul(4).toString());
    ...
    //     expect(derVfb).to.eq(ethers.BigNumber.from('1619249999999999996761'));
    //
    //     // We now do a few massive swaps to get some fees in + small syncMint to make it stick.
    //
    //     let pairResk = await pair.getReserves();
    ...
    //     console.log("Pylon Sync Reserve1 before burn: ", ethers.utils.formatEther(pylonRes[1]));
    //
    //     console.log("SENDING ===> ",  aptBalance.div(40).toString())
    //     await printValues()
    //     await pylonInstance.burn(account.address, true);
    //
    //     ptt = await pair.totalSupply();
    ...
    //     expect(balancePostBurn.sub(balancePreBurn)).to.eq(ethers.BigNumber.from('5410663904453385491'))
    //
    //     //now we dump a bit more to see if it taps into the Anchors
    //     await ethers.provider.send("hardhat_mine", ['0x30']);
    //
    ...
    //
    //     expect(ptb).to.eq(ethers.BigNumber.from("9999999999999999000"))
    //
    ...
    //
    //     console.log("SENDING", aptb.div(10).toString())
    //     await printValues()
    //     await pylonInstance.burn(account2.address, true);
    //
    ...
    it('should add async liquidity', async function () {
        ...
        console.log("FloatPTBalance before mint: ", floatBalance0.toString())
        ...
        expect(await poolTokenInstance0.balanceOf(account.address)).to.eq(ethers.BigNumber.from("170459917860391523208"))
        expect(await poolTokenInstance1.balanceOf(account.address)).to.eq(ethers.BigNumber.from("481818181818181817181"))
        ...
        // Let's check...
        expect(await poolTokenInstance0.balanceOf(account.address)).to.eq(ethers.BigNumber.from("170459917860391523208"))
        expect(await poolTokenInstance1.balanceOf(account.address)).to.eq(ethers.BigNumber.from('531668181818181817181'))
    });
    ...
    it('should dump::float', async function () {
        ...
        console.log("SENDING ", ftb.toString())
        await printValues()
        await pylonInstance.burn(account2.address, false)
