...
// const {expandTo18Decimals, getAmountOut, format, sqrt, findDeviation, calculateOmega, getFtv, expandToNDecimals} = require("./shared/utils");
// const {coreFixtures, librarySetup} = require("./shared/fixtures");
// const {
//     initPylon, initData, printState,
//     printPoolTokens, printPairState,
//     getPTPrice, burn, burnAsync,
//     forwardTime, unblockOracle, mintAsync,
//     mintSync, setPrice, updateMint,
...
//         await initPylon(fixtures, expandToNDecimals(token0Amount, 18), expandToNDecimals(token1Amount, 18), pylonPercentage)
...
//
//     /*TODO:
//         - add init parameters with low float price, high float price, low float decimals, low anchor decimals and low both
//         - "unit invariant" test + single-state conditions test (low gamma, mid-low gamma, high gamma)
//         - Oracle deep test
//         - extremes (remove all anchor, all float)
//     */
...
//
//         //Here deviation could be a lot
//         expect(deviation).to.lt(expandTo18Decimals(0.06));
//         // expect(floatsReceived).to.gt(initialFloatBalance);
//
//     });
//
...
//         pylonRes = await pylonInstance.getSyncReserves(false);
...
//
//         let pylonRes2 = await pylonInstance.getSyncReserves(false);
...
//
//         pylonRes2 = await pylonInstance.getSyncReserves(false);
