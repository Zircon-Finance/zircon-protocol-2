...
describe("Pylon Router", () => {
    ...
    it('should add sync liquidity & stake', async function () {
        ...
        await router.init(
            ...
            account.address,
            ethers.constants.MaxUint256)
        await poolTokenInstance1.approve(farm.address, ethers.constants.MaxUint256)
        await router.addSyncLiquidity(
            token0.address,
            token1.address,
            ethers.BigNumber.from('44999999999999929'),
        ...
        expect(await poolTokenInstance1.balanceOf(account.address)).to.eq(ethers.BigNumber.from('0'))
        expect(user[0]).to.eq(ethers.BigNumber.from('2044999999999998929'))
    });
